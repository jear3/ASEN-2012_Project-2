%{
    UPDATE: DOES NOT WORK DUE TO UNSUBSTANTIAL CHANGE IN INITIAL VOLUME. 
    INITIAL VOLUME OF WATER CANNOT BE VARIED INDIVIDUALLY AS IT IS A
    DEPENDENT VARIABLE. 
...
% attach variables 
Volume0 = linspace(0.0001, 0.001, 100); % Initial Volume
...
% Create a matrix that accounts for the change in distance and height travelled with respect to the Initial Volume of Water 
dataVolume0_MaxDist = ones(length(Volume0), 1);
dataVolume0_MaxHeight= ones(length(Volume0), 1);

% Get data readings
for i = 1:length(Volume0)
    const.Vi_water = Volume0(i);
    [~, state] = ode45(@(t, statevector) rocket_phases(t, statevector, const), tspan, statevector_0);
    dataVolume0_MaxDist(i) = max(state(:, 1));
    dataVolume0_MaxHeight(i) = max(state(:, 3));
end

%% Plot dataVolume0 vs Volume0
figure;
plot(dataVolume0_MaxDist, Volume0, 'Color', '[0.4940, 0.1840, 0.5560]', 'LineWidth', 2, LineStyle='-')
hold on;

% Define x value at which the vertical line is needed
requiredDistance = 92;
% Interpolate the y-value (Pressure) for x = 92
y_interp = interp1(dataVolume0_MaxDist, Volume0, requiredDistance, 'linear', 'extrap');
...
correspondingHeight = interp1(dataVolume0_MaxDist, dataVolume0_MaxHeight, requiredDistance, 'linear', 'extrap');
correspondingPressure0= y_interp;

% Display Results
fprintf("Required Max Distance: %2.1f ", requiredDistance);
fprintf("\n\nPredicted Corresponding Initial Volume: %2.3f ", correspondingPressure0);
fprintf("\nPredicted Corresponding Max Height: %2.1f ", correspondingHeight);

% plots
xlabel('Distance (m)');
ylabel('Initial Volume (m\^3)');
title('Change in Distance Travelled vs Initial Volume');
xlim([40 130])
ylim([0.0004 0.003])
xticks([50 75 92 110])
yticks([0.0005, y_interp, 0.002])
yline(0.0005, '--', 'Color', 'blue', Label={'Minimum Volume'});
yline(y_interp, '--', 'Color', [0.4660, 0.6740, 0.1880], Label={'Extrapolated Value'});
yline(0.002, '--', 'Color', 'blue', Label={'Maximum Pressure'});
grid on;
hold off;
...
%% Plot Trajectory and Thrust Profile With The Predicted Value of Initial Volume 

% Run ODE with predicted initial Volume 
const.Vi_water = y_interp; % Volume prediction 
[t, state] = ode45(@(t, statevector) rocket_phases(t, statevector, const), tspan, statevector_0); 

Thrust = zeros(length(t), 1);
for i = 1:length(t)
    const.Vi_water = y_interp; % change the initial Volume to the predicted value of v0
